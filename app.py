    ...
    with col2:
        ...
        with tab_login:
            ...
            if st.button("Log In", use_container_width=True):
                _valid_email = bool(_re.match(r"^[\w.%+\-]+@[\w.\-]+\.[a-zA-Z]{2,}$", email.strip()))
                if not email or not password:
                    st.error("\u274c Please enter your email and password.")
                elif not _valid_email:
                    st.error("\u274c Please enter a valid email address  e.g. name@gmail.com")
                elif email.strip().lower() in st.session_state.users_db and st.session_state.users_db[email.strip().lower()] == password:
                    st.session_state.logged_in = True
                    st.rerun()
                else:
        ...
        with tab_signup:
            import re as _re
            new_email    = st.text_input("Email",            key="signup_email",   placeholder="name@example.com")
            new_password = st.text_input("Password",         type="password", key="signup_pass",    placeholder="At least 6 characters")
            ...
            if st.button("Create Account", use_container_width=True):
                ...
                elif not _re.match(_ep, new_email.strip()):
                    st.error("\u274c Invalid email — please use a real email address.  e.g.  name@gmail.com")
                elif len(new_password) < 6:
                    st.error("\u274c Password must be at least 6 characters long.")
                elif new_password != confirm_pass:
                    st.error("\u274c Passwords do not match.")
                elif new_email.strip().lower() in st.session_state.users_db:
                    st.error("\u274c An account with this email already exists.")
                else:
                    st.session_state.users_db[new_email.strip().lower()] = new_password
