...
# FIX 1: Generate oauth_state only once per session to avoid CSRF mismatch
...
# FIX 2: Corrected TMDB URL construction
def get_poster(movie_id: int) -> str:
    ...
    try:
        ...
        result = (POSTER_BASE + path) if path else NO_POSTER
    except:
        result = NO_POSTER
    _pc[movie_id] = result
...
if not st.session_state.logged_in:
    ...
    # FIX 3: Handle Google OAuth callback BEFORE rendering login UI
    qp          = st.query_params
    ...
    if oauth_code:
        ...
        if stored_state and oauth_state == stored_state:
            ...
            if user_info and user_info.get("email"):
                ...
                # FIX 4: Clear oauth_state after successful login
            ...
            st.error("⚠️ OAuth state mismatch — possible CSRF. Please try again.")
            st.session_state["oauth_state"] = None
            st.query_params.clear()
    ...
    <div style="text-align:center; padding: 4rem 0 2rem;">
      ...
      <div style="font-size:1rem; color:#6b7194; margin-bottom:2rem">Your AI-powered movie companion</div>
    </div>
    """, unsafe_allow_html=True)
    ...
    with col2:
        ...
        if google_configured:
            ...
            st.markdown(f"""
            <a href="{google_url}" style="text-decoration:none; display:block; margin-bottom:1rem">
              <div style="
                display:flex; align-items:center; justify-content:center; gap:12px;
                background:#fff; color:#3c4043;
                border:1px solid #dadce0; border-radius:10px;
                padding:0.7rem 1.4rem; font-family:'Poppins',sans-serif;
            ...
            <div style="display:flex;align-items:center;gap:8px;margin:0 0 1rem">
            ...
            st.warning("💡 Set `GOOGLE_CLIENT_ID` and `GOOGLE_CLIENT_SECRET` in your `.env` to enable Google Sign-In.")
...
if st.session_state.show_account:
    with st.container(border=True):
        ...
        with c1:
            ...
            all_g    = sorted(['Action','Adventure','Animation','Comedy','Crime','Documentary',
                               'Drama','Family','Fantasy','History','Horror','Music','Mystery',
                               'Romance','Science Fiction','TV Movie','Thriller','War','Western'])
            new_genres = st.multiselect("Favorite Genres", all_g,
                                        default=st.session_state.user_profile.get("genres", []))
...
with tab_home:
    ...
    <div class="hero-panel">
      ...
      <div class="hero-sub">Pick a film of your choice, and we'll find you the perfect match</div>
    </div>
    """, unsafe_allow_html=True)
    ...
    if st.session_state.recs:
        ...
        base_row     = movies[movies['title'] == base]
        base_mid     = int(base_row['movie_id'].values[0]) if not base_row.empty else 0
        base_details = get_details(base_mid, fallback=base)
        ...
        <div class="feature-strip">
          ...
          <div class="feature-strip-text">
            CineMatrix Intelligence — <b>10 Similar Picks</b> for
            <b>{base}</b> &nbsp;·&nbsp; Genres: {genres_preview}
          </div>
        </div>
        """, unsafe_allow_html=True)

        def render_movie_row(recs_slice, label, count_label):
            st.markdown(f"""
            <div class="sec-head">
            ...
            for idx, (col, rec) in enumerate(zip(cols, recs_slice)):
                with col:
                    ...
                    if poster_url == NO_POSTER or not poster_url:
                        img_html = f"""<div style="aspect-ratio:2/3;display:flex;align-items:center;
                          justify-content:center;background:linear-gradient(135deg,#12121a,#1a1a26);width:100%">
                          <div style="font-size:1.5rem;font-weight:700;color:#f5c518;text-align:center;line-height:1.2">
                            Movie<br>🎬</div></div>"""
                    else:
                    ...
                    if st.button("Details", key=f"det_{label[:3]}_{idx}_{rec['movie_id']}", use_container_width=True):
                        d["poster"] = poster_url
                        st.session_state.detail_movie = d
                        st.switch_page("pages/movie_detail.py")

        render_movie_row(
            st.session_state.recs[:5],
            f"Because you liked <em style='color:#a78bfa'>{base}</em>",
            "1–5 of 10"
        )
        render_movie_row(
            st.session_state.recs[5:],
...
with tab_analytics:
    ...
    df_raw_full = load_raw_for_charts()
    ...
    with fc1:
        rating_range = st.slider("⭐ IMDb Rating Range", 0.0, 10.0, (0.0, 10.0), step=0.5)
    with fc2:
    ...
    with col_a:
        st.markdown("""<div class="sec-head"><div class="sec-head-bar"></div>
        <div class="sec-head-title">🎭 Top 10 Genres — Movie Count</div></div>""", unsafe_allow_html=True)
        if not top10_df.empty:
            fig1 = px.bar(top10_df.sort_values('Count', ascending=True),
                          x='Count', y='Genre', orientation='h', text='Count',
    ...
    with col_b:
        st.markdown("""<div class="sec-head"><div class="sec-head-bar"></div>
        <div class="sec-head-title">⭐ Rating Distribution</div></div>""", unsafe_allow_html=True)
        if not df_raw.empty:
            fig2 = px.histogram(df_raw.dropna(subset=['vote_average']),
                                x='vote_average', nbins=20, color_discrete_sequence=['#7c3aed'])
            mean_val = df_raw['vote_average'].mean()
            fig2.add_vline(x=mean_val, line_dash="dash", line_color="#f59e0b",
                           annotation_text=f"Mean: {mean_val:.1f}")
            fig2.update_layout(template="plotly_dark", plot_bgcolor='rgba(0,0,0,0)',
                               paper_bgcolor='rgba(0,0,0,0)', margin=dict(l=0, r=0, t=10, b=0),
                               xaxis_title="IMDb Rating", yaxis_title="Count")
    ...
    with col_c:
        st.markdown("""<div class="sec-head"><div class="sec-head-bar"></div>
        <div class="sec-head-title">📅 Movies Released per Year (1990–2017)</div></div>""", unsafe_allow_html=True)
        ...
        if not year_df.empty:
            ...
            fig3.add_trace(go.Scatter(x=year_df['year'], y=year_df['count'], fill='tozeroy',
                                      name='Movies Released', line=dict(color='#7c3aed')), secondary_y=False)
            fig3.add_trace(go.Scatter(x=year_df['year'], y=year_df['avg_rating'], name='Avg Rating',
                                      line=dict(color='#f59e0b', dash='dash')), secondary_y=True)
    ...
    with col_d:
        st.markdown("""<div class="sec-head"><div class="sec-head-bar"></div>
        <div class="sec-head-title">🍕 Top 8 Genre Share</div></div>""", unsafe_allow_html=True)
        pie8 = genre_df.head(8)
        if not pie8.empty:
            fig4 = px.pie(pie8, values='Count', names='Genre', hole=0.52,
            ...
            fig4.update_layout(template="plotly_dark", plot_bgcolor='rgba(0,0,0,0)',
                               paper_bgcolor='rgba(0,0,0,0)', margin=dict(l=0, r=0, t=10, b=0),
                               showlegend=False,
                               annotations=[dict(text=f'{pie8["Count"].sum():,}<br>films',
    ...
    st.markdown("""<div class="sec-head" style="margin-top:1rem"><div class="sec-head-bar"></div>
    <div class="sec-head-title">🔢 Top 10 Genres — Interactive Bar</div></div>""", unsafe_allow_html=True)
    if not top10_df.empty:
        st.bar_chart(top10_df.set_index('Genre')['Count'], use_container_width=True,
                     height=320, color="#7c3aed")
    else:
        st.info("No data for this filter.")
    ...
    with col_e:
        st.markdown("""<div class="sec-head"><div class="sec-head-bar"></div>
        <div class="sec-head-title">⏱ Runtime vs IMDb Rating</div></div>""", unsafe_allow_html=True)
        ...
        if not scatter_df.empty:
            fig5 = px.scatter(scatter_df, x='runtime', y='vote_average', color='vote_average',
            ...
            fig5.update_layout(template="plotly_dark", plot_bgcolor='rgba(0,0,0,0)',
                               paper_bgcolor='rgba(0,0,0,0)', margin=dict(l=0, r=0, t=10, b=0),
                               xaxis_title="Runtime (min)", yaxis_title="Rating")
    ...
    with col_f:
        st.markdown("""<div class="sec-head"><div class="sec-head-bar"></div>
        <div class="sec-head-title">💰 Budget vs Revenue (log scale)</div></div>""", unsafe_allow_html=True)
        bvr = df_raw[(df_raw['budget'] > 1e6) & (df_raw['revenue'] > 1e6)].copy()
        if not bvr.empty:
            fig6 = px.scatter(bvr, x='budget', y='revenue', color='vote_average',
            ...
            fig6.update_layout(template="plotly_dark", plot_bgcolor='rgba(0,0,0,0)',
                               paper_bgcolor='rgba(0,0,0,0)', margin=dict(l=0, r=0, t=10, b=0),
    ...
    # ── Top 10 table ──────────────────────────────────────────────────────────
    st.markdown("""<div class="sec-head" style="margin-top:1rem"><div class="sec-head-bar"></div>
    <div class="sec-head-title">🏆 Top 10 Movies (Based on Filters)</div></div>""", unsafe_allow_html=True)
    ...
    if top10_filtered.empty:
        rows_html = '<tr><td colspan="5" style="padding:1rem;text-align:center;color:#6b7194">No movies match filters.</td></tr>'
    else:
