...
# ── HMAC-signed state ─────────────────────────────────────────────────────────
# Streamlit destroys session_state on every redirect, so we can't store the
# OAuth state token there. Instead we sign a timestamp with the client secret.
# On the callback we just verify the signature — no stored value needed.
...
:root{--bg:#07070f;--surface:#0f0f1c;--card:#14142a;--border:#1e1e38;
  --border2:#2a2a50;--purple:#7c3aed;--blue:#2563eb;--cyan:#06b6d4;
  --pink:#ec4899;--gold:#f59e0b;--green:#10b981;--text:#f1f0ff;
  --muted:#6b7194;--radius:14px;}
...
.navbar{display:flex;align-items:center;justify-content:space-between;padding:1rem 0;margin-bottom:2rem;border-bottom:1px solid var(--border);}
.nav-brand{display:flex;align-items:center;gap:10px;font-size:1.5rem;font-weight:900;letter-spacing:-0.02em;}
.nav-brand-icon{width:38px;height:38px;background:linear-gradient(135deg,var(--purple) 0%,var(--blue) 100%);border-radius:10px;display:flex;align-items:center;justify-content:center;font-size:1.1rem;box-shadow:0 4px 15px rgba(124,58,237,0.45);}
.nav-brand-text{background:linear-gradient(135deg,#a78bfa 0%,#60a5fa 100%);-webkit-background-clip:text;-webkit-text-fill-color:transparent;}
...
.hero-panel{background:linear-gradient(135deg,rgba(124,58,237,0.18) 0%,rgba(37,99,235,0.14) 50%,rgba(6,182,212,0.10) 100%);border:1px solid rgba(124,58,237,0.25);border-radius:20px;padding:2.4rem 2.8rem;margin-bottom:2.5rem;position:relative;overflow:hidden;}
.hero-panel::before{content:'';position:absolute;top:-60px;right:-60px;width:250px;height:250px;background:radial-gradient(circle,rgba(124,58,237,.12) 0%,transparent 70%);border-radius:50%;}
.hero-panel::after{content:'';position:absolute;bottom:-40px;left:30%;width:180px;height:180px;background:radial-gradient(circle,rgba(6,182,212,.08) 0%,transparent 70%);border-radius:50%;}
.hero-label{font-size:0.72rem;font-weight:600;color:var(--cyan);text-transform:uppercase;letter-spacing:0.15em;margin-bottom:0.55rem;}
.hero-heading{font-size:1.8rem;font-weight:800;line-height:1.2;margin-bottom:0.4rem;color:var(--text);}
.hero-heading span{background:linear-gradient(135deg,#a78bfa 0%,#60a5fa 70%);-webkit-background-clip:text;-webkit-text-fill-color:transparent;}
.hero-sub{font-size:0.88rem;color:var(--muted);margin-bottom:0;}
.stSelectbox>div>div{background:rgba(255,255,255,0.04)!important;border:1px solid var(--border2)!important;border-radius:12px!important;color:var(--text)!important;font-size:1rem!important;font-family:'Poppins',sans-serif!important;}
.stSelectbox label{color:var(--muted)!important;font-size:0.75rem!important;text-transform:uppercase;letter-spacing:0.1em;}
div.stButton>button,div.stFormSubmitButton>button{background:linear-gradient(135deg,var(--purple) 0%,var(--blue) 100%)!important;color:#fff!important;font-weight:700!important;font-size:0.9rem!important;border:none!important;border-radius:10px!important;padding:0.65rem 1.6rem!important;letter-spacing:0.04em;font-family:'Poppins',sans-serif!important;transition:transform 0.15s,box-shadow 0.15s!important;width:100%;box-shadow:0 4px 15px rgba(124,58,237,0.35)!important;}
div.stButton>button:hover,div.stFormSubmitButton>button:hover{transform:translateY(-2px)!important;box-shadow:0 8px 25px rgba(124,58,237,0.55)!important;}
.stTabs [data-baseweb="tab-list"]{background:var(--surface)!important;border-radius:12px!important;padding:4px!important;gap:3px!important;border:1px solid var(--border)!important;}
.stTabs [data-baseweb="tab"]{color:var(--muted)!important;font-family:'Poppins',sans-serif!important;font-size:0.85rem!important;font-weight:500!important;border-radius:9px!important;padding:0.45rem 1.3rem!important;border:none!important;}
.stTabs [aria-selected="true"]{background:linear-gradient(135deg,var(--purple) 0%,var(--blue) 100%)!important;color:#fff!important;font-weight:700!important;box-shadow:0 3px 12px rgba(124,58,237,0.45)!important;}
.stSpinner>div{border-top-color:var(--purple)!important;}
hr{border-color:var(--border)!important;}
.sec-head{display:flex;align-items:center;gap:12px;margin:2.2rem 0 1.2rem;padding-bottom:0.7rem;border-bottom:1px solid var(--border);}
.sec-head-bar{width:4px;height:1.3rem;border-radius:2px;background:linear-gradient(to bottom,var(--purple),var(--cyan));}
.sec-head-title{font-size:1.1rem;font-weight:700;color:var(--text);}
.sec-head-count{margin-left:auto;font-size:0.72rem;color:var(--muted);background:var(--surface);border:1px solid var(--border);border-radius:20px;padding:2px 10px;}
.mc{background:var(--card);border:1px solid var(--border);border-radius:var(--radius);overflow:hidden;position:relative;margin-bottom:0.5rem;transition:transform 0.22s ease,border-color 0.22s ease,box-shadow 0.22s ease;cursor:pointer;}
.mc:hover{transform:translateY(-7px) scale(1.025);border-color:var(--purple);box-shadow:0 18px 45px rgba(0,0,0,0.65),0 0 0 1px rgba(124,58,237,0.5);}
.mc img{width:100%;aspect-ratio:2/3;object-fit:cover;display:block;background:var(--surface);}
.mc-title-bar{position:absolute;bottom:0;left:0;right:0;background:linear-gradient(to top,rgba(7,7,15,0.95) 0%,rgba(7,7,15,0.6) 60%,transparent 100%);padding:2rem 0.75rem 0.75rem;transition:opacity 0.3s ease;}
.mc-overlay{position:absolute;top:0;bottom:0;left:0;right:0;background:rgba(7,7,15,0.92);backdrop-filter:blur(4px);padding:1.2rem 1rem;display:flex;flex-direction:column;justify-content:center;opacity:0;transition:opacity 0.3s ease;}
.mc:hover .mc-title-bar{opacity:0;}.mc:hover .mc-overlay{opacity:1;}
.mc-title-truncate{font-size:0.78rem;font-weight:600;color:#fff;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;margin-bottom:3px;line-height:1.3;}
...
.mc-overview{font-size:0.65rem;color:#d1d1e0;line-height:1.4;display:-webkit-box;-webkit-line-clamp:5;-webkit-box-orient:vertical;overflow:hidden;}
.mc-genre-tag{position:absolute;top:8px;right:8px;background:rgba(124,58,237,0.8);backdrop-filter:blur(4px);border-radius:6px;padding:2px 7px;font-size:0.6rem;font-weight:600;color:#fff;letter-spacing:0.05em;}
.feature-strip{background:linear-gradient(135deg,var(--card) 0%,rgba(124,58,237,0.08) 100%);border:1px solid var(--border2);border-radius:16px;padding:1.2rem 1.5rem;margin-bottom:1.8rem;display:flex;align-items:center;gap:1rem;}
.feature-strip-dot{width:10px;height:10px;border-radius:50%;background:linear-gradient(135deg,var(--purple),var(--cyan));flex-shrink:0;box-shadow:0 0 8px rgba(124,58,237,0.7);animation:pulse-dot 2s infinite;}
@keyframes pulse-dot{0%,100%{box-shadow:0 0 8px rgba(124,58,237,0.7);}50%{box-shadow:0 0 18px rgba(124,58,237,1);}}
.feature-strip-text{font-size:0.9rem;color:var(--muted);}
.feature-strip-text b{color:var(--text);}
.site-footer{text-align:center;color:var(--muted);font-size:0.72rem;padding:2rem 0 1rem;border-top:1px solid var(--border);margin-top:3rem;letter-spacing:0.05em;}
...
# ── Session defaults ──────────────────────────────────────────────────────────
for k, v in [
    ("recs", []), ("searched_for", None), ("logged_in", False),
    ("users_db", {"admin@cinematrix.com": "password"}),
    ("search_history", []), ("user_profile", {"name": "", "genres": []}),
    ("show_account", False),
]:
    if k not in st.session_state:
        st.session_state[k] = v

# ── OAuth callback — runs FIRST on every page load ────────────────────────────
...
<div class="navbar">
  <div class="nav-brand">
    ...
    <div><div class="nav-brand-text">CineMatrix</div><div class="nav-subtitle">Movie Intelligence</div></div>
  </div>
  <div class="nav-stats">
    <div class="nav-stat"><div class="nav-stat-num">{len(movies):,}</div><div class="nav-stat-lbl">Movies</div></div>
    <div class="nav-stat"><div class="nav-stat-num">ML</div><div class="nav-stat-lbl">Powered</div></div>
  </div>
</div>
""", unsafe_allow_html=True)
...
with tab_home:
    ...
    if st.session_state.recs:
        ...
        <div class="feature-strip">
          ...
          <div class="feature-strip-text">CineMatrix Intelligence — <b>10 Similar Picks for {base}</b> &nbsp;·&nbsp; Genres: {gp}</div>
            ...
            for idx, (col, rec) in enumerate(zip(st.columns(5, gap="small"), recs_slice)):
                with col:
                    ...
                    st.markdown(f"""<div class="mc">{ih}
                      ...
                      <div class="mc-title-bar"><div class="mc-title-truncate">{d['title']}</div><div class="mc-rating">⭐ {d['rating']}</div></div>
                      <div class="mc-overlay"><div class="mc-title">{d['title']}</div>
...
with tab_analytics:
    ...
    st.markdown('<div class="sec-head" style="margin-top:1.5rem"><div class="sec-head-bar"></div><div class="sec-head-title">Dataset Analytics — TMDB 5,000 Movies</div></div>', unsafe_allow_html=True)
    ...
    df = df_full[(df_full["vote_average"]>=rr[0])&(df_full["vote_average"]<=rr[1])&(df_full["year"]>=yr[0])&(df_full["year"]<=yr[1])]
    ...
    _pc_kw = dict(template="plotly_dark",plot_bgcolor="rgba(0,0,0,0)",paper_bgcolor="rgba(0,0,0,0)",margin=dict(l=0,r=0,t=30,b=0))
    ...
    with ca:
        ...
        if not top10.empty:
            fig=px.bar(top10.sort_values("Count",ascending=True),x="Count",y="Genre",orientation="h",text="Count",color_discrete_sequence=["#7c3aed"])
            fig.update_traces(textposition="outside"); fig.update_layout(**_pc_kw,xaxis_title="Number of Movies",yaxis_title="")
    ...
    with cb:
        ...
        if not df.empty:
            ...
            mv=df["vote_average"].mean(); fig.add_vline(x=mv,line_dash="dash",line_color="#f59e0b",annotation_text=f"Mean:{mv:.1f}")
            fig.update_layout(**_pc_kw,xaxis_title="IMDb Rating",yaxis_title="Count"); st.plotly_chart(fig,use_container_width=True)
    ...
    with cc:
        ...
        ydf=df[df["year"].between(1990,2017)].groupby("year").agg(count=("title","count"),avg_rating=("vote_average","mean")).reset_index()
        if not ydf.empty:
            ...
            fig.update_yaxes(title_text="# Movies",secondary_y=False); fig.update_yaxes(title_text="Avg Rating",secondary_y=True)
            st.plotly_chart(fig,use_container_width=True)
    with cd:
        ...
        p8=gdf.head(8)
        if not p8.empty:
            fig=px.pie(p8,values="Count",names="Genre",hole=0.52,color_discrete_sequence=["#7c3aed","#2563eb","#06b6d4","#10b981","#f59e0b","#ec4899","#6d28d9","#1d4ed8"])
            fig.update_traces(textposition="inside",textinfo="percent+label")
            fig.update_layout(**_pc_kw,showlegend=False,annotations=[dict(text=f'{p8["Count"].sum():,}<br>films',x=0.5,y=0.5,font_size=14,showarrow=False,font_color="#a78bfa")])
            st.plotly_chart(fig,use_container_width=True)
    ...
    with ce:
        ...
        sc=df[["runtime","vote_average","title"]].dropna(); sc=sc[(sc["runtime"]>40)&(sc["runtime"]<240)]
        if not sc.empty:
            fig=px.scatter(sc,x="runtime",y="vote_average",color="vote_average",hover_name="title",color_continuous_scale="plasma",opacity=0.7)
            fig.update_layout(**_pc_kw,xaxis_title="Runtime (min)",yaxis_title="Rating"); st.plotly_chart(fig,use_container_width=True)
    with cf:
        ...
        if not bvr.empty:
            fig=px.scatter(bvr,x="budget",y="revenue",color="vote_average",hover_name="title",color_continuous_scale="plasma",log_x=True,log_y=True,opacity=0.7)
            mn,mx=min(bvr["budget"].min(),bvr["revenue"].min()),max(bvr["budget"].max(),bvr["revenue"].max())
            fig.add_shape(type="line",x0=mn,y0=mn,x1=mx,y1=mx,line=dict(color="#f59e0b",dash="dash",width=1))
            fig.update_layout(**_pc_kw,xaxis_title="Budget ($)",yaxis_title="Revenue ($)"); st.plotly_chart(fig,use_container_width=True)

    st.markdown('<div class="sec-head" style="margin-top:1rem"><div class="sec-head-bar"></div><div class="sec-head-title">🏆 Top 10 Movies (Filtered)</div></div>',unsafe_allow_html=True)
    mv_req=500 if len(df)>500 else 10
    top10f=df[df["vote_count"]>=mv_req].nlargest(10,"vote_average")[["title","vote_average","vote_count","year"]].reset_index(drop=True)
    top10f.index+=1; top10f.columns=["Title","Rating","Votes","Year"]; top10f["Year"]=top10f["Year"].astype("Int64")
    ...
    for i,row in top10f.iterrows():
        ...
        rh+=f'<tr style="border-bottom:1px solid #1e1e38"><td style="padding:.55rem .7rem;color:#a78bfa;font-weight:700">{i}</td><td style="padding:.55rem .7rem;color:#f1f0ff;font-weight:500">{row["Title"]}</td><td style="padding:.55rem .7rem"><div style="display:flex;align-items:center;gap:8px"><div style="width:80px;background:#1e1e38;border-radius:4px;height:6px;overflow:hidden"><div style="width:{bw}%;background:linear-gradient(90deg,#7c3aed,#06b6d4);height:100%;border-radius:4px"></div></div><span style="color:#f59e0b;font-weight:700;font-size:.82rem">{row["Rating"]}</span></div></td><td style="padding:.55rem .7rem;color:#6b7194;font-size:.82rem">{row["Votes"]:,}</td><td style="padding:.55rem .7rem;color:#6b7194;font-size:.82rem">{row["Year"]}</td></tr>'
    st.markdown(f'<div style="background:var(--card);border:1px solid var(--border);border-radius:14px;overflow:hidden"><table style="width:100%;border-collapse:collapse;font-family:\'Poppins\',sans-serif;font-size:.85rem"><thead><tr style="background:#0f0f1c;border-bottom:2px solid #2a2a50"><th style="padding:.65rem .7rem;color:#6b7194;font-weight:600;text-align:left">#</th><th style="padding:.65rem .7rem;color:#6b7194;font-weight:600;text-align:left">Title</th><th style="padding:.65rem .7rem;color:#6b7194;font-weight:600;text-align:left">Rating</th><th style="padding:.65rem .7rem;color:#6b7194;font-weight:600;text-align:left">Votes</th><th style="padding:.65rem .7rem;color:#6b7194;font-weight:600;text-align:left">Year</th></tr></thead><tbody>{rh}</tbody></table></div>',unsafe_allow_html=True)

st.markdown('<div class="site-footer"><span>CineMatrix</span> &nbsp;·&nbsp; Powered by TMDB &nbsp;·&nbsp; Built with Streamlit &amp; Python &nbsp;·&nbsp; © 2025</div>',unsafe_allow_html=True)
